...
///Retourne le mot, le score et le nombre de stars obtenues pour ce mot
(String word, int score, int starsUsed) _extractWordWithScore(
    ...
    if (isNewTile) {
      switch (bonus) {
        ...
        case BonusType.tripleWord:
          ...
          break;
        case BonusType.star:
          // ✅ Seulement incrémenter les étoiles, pas de multiplicateur de score
          starsUsed++;
          break;
        case BonusType.none:
          break;
  ...
  return (word, wordScore * wordMultiplier, starsUsed);
}

// ============================================================
// MODIFICATION : getWordsCreatedAndScore retourne maintenant aussi les étoiles
// ============================================================
({List<String> words, int totalScore, int totalStarsUsed})
getWordsCreatedAndScore({
  ...
  if (lettersPlacedThisTurn.isEmpty) {
    return (words: [], totalScore: 0, totalStarsUsed: 0);
  }

  final words = <String>{};
  int totalScore = 0;
  int totalStarsUsed = 0; // NOUVEAU
  ...
  final (mainWord, mainScore, mainStars) = _extractWordWithScore(
    board,
    bonusMap,
    start.row,
    start.col,
  ...
  if (mainWord.length > 1) {
    ...
    totalStarsUsed += mainStars; // AJOUTER LES ÉTOILES DU MOT PRINCIPAL
  }

  // --- Mots secondaires ---
  for (final l in lettersPlacedThisTurn) {
    final (perpWord, perpScore, perpStars) = _extractWordWithScore(
      board,
      bonusMap,
      l.row,
      l.col,
    ...
    if (perpWord.length > 1) {
      ...
      totalStarsUsed += perpStars; // AJOUTER LES ÉTOILES DES MOTS SECONDAIRES
    }
  }
...
// ============================================================
// MODIFICATION : getWordAtPosition avec gestion des étoiles
// ============================================================
String? getWordAtPosition({
  ...
  final (horizontalWord, _, _) = _extractWordWithScore(
    board,
    bonusMap,
    row,
    col,
  ...
  final (verticalWord, _, _) = _extractWordWithScore(
    board,
    bonusMap,
    row,
    col,
