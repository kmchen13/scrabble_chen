...
class ChangeLettersDialog extends StatefulWidget {
  ...
  final bool cancelRemovesLast;
  final ValueChanged<List<String>> onConfirm;

  const ChangeLettersDialog({
    Key? key,
    required this.playerLetters,
    required this.bag,
    required this.title,
    this.description = '',
    this.confirmText = 'Changer',
    this.allowEmptySelection = true,
    this.cancelRemovesLast = false,
    required this.onConfirm,
  }) : super(key: key);

  @override
  State<ChangeLettersDialog> createState() => _ChangeLettersDialogState();
}

class _ChangeLettersDialogState extends State<ChangeLettersDialog> {
  ...
  void _handleCancel() {
    if (widget.cancelRemovesLast && _lettersToRemove.isNotEmpty) {
      setState(() {
  ...
  void _handleConfirm() {
    // Si aucune lettre sélectionnée, on confirme sans changer
    if (_lettersToRemove.isEmpty) {
      Navigator.of(context).pop();
      widget.onConfirm(_playerLetters);
      return;
    }
    ...
    // Fermer et retourner les nouvelles lettres
    Navigator.of(context).pop();
    widget.onConfirm(_playerLetters);
  }
  ...
  Widget build(BuildContext context) {
    final canConfirm =
        widget.allowEmptySelection || _lettersToRemove.isNotEmpty;
              ...
              children: [
                ...
                Expanded(
                  child: OutlinedButton(
                    ...
                    child: Text(
                      widget.cancelRemovesLast && _lettersToRemove.isNotEmpty
                          ? 'Annuler'
                          : 'Fermer',
